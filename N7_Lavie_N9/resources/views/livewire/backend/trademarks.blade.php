<div class="page-content">
    <div class="row">
        <div class="col-4">
            <div class="card">
                <div class="card-body">
                    <h6 class="mb-0 text-uppercase text-primary">Thuong hieu</h6>
                    <hr>
                    <form
                    ...
                    >
                        <div class="col">
                            ...
                            @if($editID)
                                <input type="text" class="form-control" placeholder="Nhập tên thể loại" wire:model="editName" wire:keyup="generateSlug">
                            @else
                                <input type="text" class="form-control @error('name') is-invalid @enderror" placeholder="Tên thể loại" wire:model="name" wire:keyup="generateSlug" autofocus>
                        ...
                        <div class="col">
                            <label>Mô tả</label>
                            @if($editID)
                                <textarea class="form-control" rows="10" placeholder="Mô tả thể loại" wire:model="editDescription"></textarea>
                            @else
                                <textarea class="form-control" rows="10" placeholder="Mô tả thể loại" wire:model="description"></textarea>
        ...
        <div class="col-8">
            <h6 class="mb-0 text-uppercase">Thể loại</h6>
            <hr/>
            <div class="card">
                <div class="card-body">
                    <div class="row mb-2">
    ...
    {{--                            <th>Ảnh đại diện</th>--}}
                        ...
                        @foreach ($trademarks as $trademark)
                            <tr wire:key="{{ $trademark->id }}">
                                <td>{{ $loop->index + 1 }}</td>
                                <td>{{ $trademark->name ?? 'N/A' }}</td>
                                <td>{{ $trademark->description }}</td>
                                <td>{{ $trademark->slug }}</td>
{{--                                <td><img src="{{ asset('/storage/' . $trademark->thumbnail ) }}" class="img-fluid"></td>--}}
                                <td>
                                    <button class="btn btn-sm btn-light radius-30 text-warning" wire:click="edit({{ $trademark->id }})"><i class="bx bxs-edit"></i></button>
                                </td>
                                <td>
                                    <button class="btn btn-sm btn-light radius-30 text-danger" wire:click="delete({{ $trademark->id }})" wire:confirm="Bạn chắc chắn muốn xóa"><i class="bx bxs-trash"></i></button>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-12 col-md-7">{{ $trademarks->links() }}</div>
                    </div>
                </div>
            </div>
        </div>
