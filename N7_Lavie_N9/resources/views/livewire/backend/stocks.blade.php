<div class="page-content">
    <div class="row">
        <div class="col-4">
            <div class="card">
                <div class="card-body">
                    <h6 class="mb-0 text-uppercase text-primary">Kho hàng</h6>
                    <hr>
                    <form
                    ...
                    >
                        <div class="col">
                            ...
                            @if($editID)
                                <input type="text" class="form-control" placeholder="Nhập tên kho hàng" wire:model="editName" wire:keyup="generateSlug">
                            @else
                                <input type="text" class="form-control @error('name') is-invalid @enderror" placeholder="Tên kho hàng" wire:model="name" wire:keyup="generateSlug" autofocus>
                        ...
                        <div class="col">
                            <label>Mô tả</label>
                            @if($editID)
                                <textarea class="form-control" rows="10" placeholder="Mô tả kho hàng" wire:model="editDescription"></textarea>
                            @else
                                <textarea class="form-control" rows="10" placeholder="Mô tả kho hàng" wire:model="description"></textarea>
                                @error('description') <span class="text-danger">{{ $message }}</span> @enderror
                            @endif
                        </div>
                        {{--<div class="col">
                            <label for="image" class="form-label">Ảnh đại diện kho hàng</label>
                            @if($editID)
        ...
        <div class="col-8">
            <h6 class="mb-0 text-uppercase">Kho hàng</h6>
            <hr/>
            <div class="card">
                <div class="card-body">
                    <div class="row mb-2">
                    ...
                    <table class="table table-striped">
                        ...
                        @foreach ($stocks as $stock)
                            <tr wire:key="{{ $stock->id }}">
                                ...
                                <td>{{ $stock->date_input }}</td>
                                <td>{{ $stock->date_out }}</td>
                                <td>{{ $stock->status_id }}</td>
                                <td>{{ $stock->staff_id }}</td>
                                <td>{{ $stock->customer_id }}</td>
                                <td>{{ $stock->inventory }}</td>
                                <td>{{ $stock->product_inventory }}</td>
                                <td>
                                    <button class="btn btn-sm btn-light radius-30 text-danger" wire:click="delete({{ $stock->id }})" wire:confirm="Bạn chắc chắn muốn xóa"><i class="bx bxs-trash"></i></button>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
