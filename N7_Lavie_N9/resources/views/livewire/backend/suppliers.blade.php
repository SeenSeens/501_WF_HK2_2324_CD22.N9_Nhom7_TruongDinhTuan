<div class="page-content">
    <div class="row">
        <div class="col-4">
            <div class="card">
                <div class="card-body">
                    <h6 class="mb-0 text-uppercase text-primary">Nhà cung cấp</h6>
                    <hr>
                    <form
                    ...
                    >
                        <div class="col">
                            ...
                            @if($editID)
                                <input type="text" class="form-control" placeholder="Nhập tên nhà cung cấp" wire:model="editName" wire:keyup="generateSlug">
                            @else
                                <input type="text" class="form-control @error('name') is-invalid @enderror" placeholder="Tên nhà cung cấp" wire:model="name" wire:keyup="generateSlug" autofocus>
                        ...
                        <div class="col">
                            <label>Mô tả</label>
                            @if($editID)
                                <textarea class="form-control" rows="10" placeholder="Mô tả nhà cung cấp" wire:model="editDescription"></textarea>
                            @else
                                <textarea class="form-control" rows="10" placeholder="Mô tả nhà cung cấp" wire:model="description"></textarea>
                                @error('description') <span class="text-danger">{{ $message }}</span> @enderror
                            @endif
                        </div>
                        {{--<div class="col">
                            <label for="image" class="form-label">Ảnh đại diện nhà cung cấp</label>
                            @if($editID)
        ...
        <div class="col-8">
            <h6 class="mb-0 text-uppercase">Nhà cung cấp</h6>
            <hr/>
            <div class="card">
                <div class="card-body">
                    <div class="row mb-2">
                    ...
                    <table class="table table-striped">
                        ...
                        @foreach ($suppliers as $supplier)
                            <tr wire:key="{{ $supplier->id }}">
                                ...
                                <td>{{ $supplier->slug }}</td>
                                <td><img src="{{ asset('/storage/' . $supplier->thumbnail ) }}" class="img-fluid"></td>
                                <td>
                                    <button class="btn btn-sm btn-light radius-30 text-warning" wire:click="edit({{ $supplier->id }})"><i class="bx bxs-edit"></i></button>
                                </td>
                                <td>
                                    <button class="btn btn-sm btn-light radius-30 text-danger" wire:click="delete({{ $supplier->id }})" wire:confirm="Bạn chắc chắn muốn xóa"><i class="bx bxs-trash"></i></button>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
